...
|----------------------|------------|-------------|
| **Name**              | _\*mandatory_ | Unique name of the workflow. The workflow name should be easily identifiable for your reference at a later stage. For example - `Appointment Reminder`. You can also use event name as your workflow name if there is only one workflow linked to that event. |
| **Description**       | _optional_  | You can use this field to add more details about your workflow like the notification logic. For example - `Send 1 hour before the appointment`. |
...
| **Tags**              | _optional_  | Tags are used to group related workflows together. For example, all booking-related workflows can be tagged as `Booking`. |
