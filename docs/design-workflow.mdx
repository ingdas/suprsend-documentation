...
* `event name`
   ...
   \- Name of the event defined in the workflow trigger.

* `properties`

   \- Properties should have data corresponding to variables added in the template content or data required to render dynamic fields in workflow nodes like override channels, dynamic delay windows, etc.

* `distinct_id`

  \- Unique identifier of the user who performs the event and needs to be notified. You can also 

  [override recipient](/docs/override-recipient-list)

   to notify other users who have not performed the event. For instance, notify the document owner of the new comments added by a commentor or inform the finance team when a new expense ticket is raised.
...
1. [Delay](/docs/delay)
   ...
   : wait for a time before proceeding to the next node.

2. [Batch](/docs/batching-digest)

   : aggregate multiple triggers into a single batch and send a single consolidated notification.
...
1. [Wait Until](/docs/wait-until)

   : wait until a condition is met or a specific time interval is reached.
