...
<CardGroup cols="2">
  ...
  <Card title="Update Object Subscription" href="/docs/subcribe-unsubscribe-from-object">
...
| Field                 | Description |
| --------------------- | ---------- |
| Name                  | Unique name of the workflow in human-readable format. For example - `Appointment Reminder`. Choose a relevant name initially as it will determine the related workflow slug, which cannot be modified later.            |
| Description           | You can use this field to add more details about your workflow like the notification logic. For example - Send 1 hour before the appointment           |
| Notification Category |  [Notification categories]("/docs/notification-category") define execution priority and apply user preferences during sending. There are three main categories: transactional (user action-based alerts), promotional (marketing notifications), and system (time-sensitive alerts like OTPs and authentication codes). Assign your workflow to the right category as it ensures the order of notification processing—system > transactional > promotional.       |
| Tags                  | Tags are used to group related workflows together. For example, all booking related workflows can be tagged as `Booking`.           |      
| Throttle              |You can use [throttle]("/docs/throttle") to limit the number of workflow executions per user in a given time window.| 
