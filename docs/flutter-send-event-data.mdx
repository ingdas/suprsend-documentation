...
[Create User](/docs/flutter-create-user) - Mandatory to pass in event trigger

## Sending Events to SuprSend

Events can be used to trigger workflows created on SuprSend dashboard. You can send Events from your app to SuprSend platform by using `suprsend.track()` method. Event properties can be used to pass dynamic data in trigger to apply logic in the workflow or replace template variables.
