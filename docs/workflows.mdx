...
1. **[Trigger](/docs/trigger-workflow):&#x20;**&#x54;his is the first node in your workflow that initiates the notification. You can trigger workflow using an event call or a direct API call to SuprSend. Events can be

   1. **External actions performed by a user on your platform**, such as `post like` or `a new comment`. You can track these actions by integrating one of our [frontend SDKs](/docs/javascript-sdk) directly into your product or pass them via [backend SDKs](/docs/python-sdk) or [HTTP API](/reference/event-api).
   2. **User entering or leaving a list**: these events are generated when you maintain a user list and you want to notify users who leave or enter the list, for instance, when `users unsubscribe from a topic`. These events are [auto tracked](/docs/lists#configure-list-events) if you maintain lists within SuprSend.
...
* [Design your first workflow](https://docs.suprsend.com/docs/design-workflow)

* [Table of Contents](#)

* * [Notification Categories](#notification-categories)
  * [Designing a workflow](#designing-a-workflow)
  * [Version control for workflows](#version-control-for-workflows)
  * [Triggering a workflow](#triggering-a-workflow)
  * [Triggering workflow via API](#triggering-workflow-via-api)
    * [Event based Trigger](#event-based-trigger)
  * [Send notification using Google Sheets](#send-notification-using-google-sheets)
  * [Per-Tenant workflow](#per-tenant-workflow)
  * [Tracking / Debugging Workflow run](#tracking--debugging-workflow-run)
  * [Analyzing notification performance](#analyzing-notification-performance)
