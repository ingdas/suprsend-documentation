...
<Steps>
  ...
  <Step title="Accumulates trigger until the next digest schedule">
  ...
  <Step title="Moves to next workflow step">
...
<AccordionGroup>
  <Accordion title="Fixed Schedule">
  ...
  <Accordion title="Dynamic Schedule (Send Digest based on user preference)">
    ...
    |          |                                       |             |
    |          | string (optional)                     |             |
    |          | datetime (ISO-8601 format) (optional) |             |
    |          | string (optional)                     |             |
    |          |                                       |             |
...
```javascript Event trigger Payload
//Event 1
...
<AccordionGroup>
  <Accordion title="Sending immediate or digest notification based on user's preference">
  ...
  <Accordion title="Deliver individual alerts to users throughout the day and provide a summary of all alerts at day’s end">
  ...
  <Accordion title="Send recommendation or top stories at the end of the day">
    Club with fetch node to fetch the latest recommendations before sending the digest.
  </Accordion>
  <Accordion title="Send batched reminders for pending activities or courses">
