...
<Warning>
  After calling `add\_\*/remove\_\*/unset` methods, don't forget to call `users.async_edit()` since user edit is async update and the changes will be sent to SuprSend only after calling this method.
</Warning>
