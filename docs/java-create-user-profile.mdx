...
// Instantiate user
String distinctId = "__distinct_id__";
Subscriber user = suprsendClient.user.getEditInstance(distinctId);
...
// Save Changes
JSONObject res = suprsendClient.users.asyncEdit(user);
...
<AccordionGroup>
  <Accordion title="Add User Channels" icon="sparkles">
  ...
  <Accordion title="Remove User Channels" icon="sparkles">
...
<Warning>
  After calling `add\_\*/remove\_\*/unset` methods, don't forget to call `JSONObject res = suprsendClient.users.asyncEdit(user);` since user edit is async update and the changes will be sent to SuprSend only after calling this method.
</Warning>
...
The expected response will be an instance of `suprsend.BulkResponse` class
