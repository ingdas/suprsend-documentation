...
<Note>
  **Note: **EventName in Track event method should exactly match the Event name added in Workflow configurations
</Note>
...
Use `.Append()` on bulk_events instance to add however-many-records to call in bulk. Response is an instance of `suprsend.BulkResponse` type
