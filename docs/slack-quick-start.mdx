...
[Suggest Edits](/edit/slack-quick-start)
...
* **Template** in delivery node contains the content of the notification. You can add both static and dynamic content sourced from user properties or trigger payloads. We use [JSONNET](https://jsonnet.org/ref/language.html) as Slack templating language. You can add dynamic content as `data.var` or `data["$batched_events"][0].var` (for batched alerts).\
  Ensure to publish the template before using it in a workflow. [Learn how to design Slack template here](/docs/slack-template).
...
* Here is a blog that may come in handy while designing your Slack notification:
  * [Customising Transactional Notifications using Slack Block Kit and JSONNET](https://www.suprsend.com/post/customising-transactional-notifications-using-slack-block-kit-and-jsonnet-with-examples-from-jira)
...
* * [1. Create SuprSend account](#1-create-suprsend-account)
  * [2. Integrate Slack and add option to join Slack in your product](#2-integrate-slack-and-add-option-to-join-slack-in-your-product)
