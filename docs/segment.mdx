---
title: "Segment"
description: "Guide to integrate Segment with SuprSend"
---

[Suggest Edits](/edit/segment)
...
<Warning>
  Language is stored in ISO 639-1 standard language codes. So, if you pass any value in language which is not in this format, it will be ignored. You can refer to all the [language codes here](https://github.com/suprsend/suprsend-py-sdk/blob/main/src/suprsend/language_codes.py)
</Warning>
...
2. This will open destinations catalog. Search for webhook and select **"Webhooks (Actions)"**

![](/images/docs/914e86b-image.png)

3. On the Webhooks page, click on **"Add destination"**

![](/images/docs/c2d4b57-image.png)

4. Select the data source from which you are syncing your user and event data and click on "Next"
5. On the setup screen, add Destination name. Destination name can be **"SuprSend \{workspace}"**. For example - For staging workspace, the destination name can be "SuprSend Staging". Choose **"Fill in settings manually"** and click on **"Create destination"**.

![](/images/docs/e27856c-image.png)

6. Now, on your webhook destination page, go to **Mappings** tab and create a **+New Mapping**.

![](/images/docs/ebd8075-image.png)

7. Select **Send an HTTP request** in Add Mapping modal and add below details

![](/images/docs/be129b0-image.png)

7. Add below mapping in the send request:

1. Select **any of event type - track and identify**

![](/images/docs/71ff24e-image.png)

2. In Add test event, select **Load Test Event from Source**

![](/images/docs/141834f-image.png)

3. In Select Mappings, add below details:

   1. `https://hub.suprsend.com/connector/segment/` -> URL
   2. `POST` -> Method
   3. `Bearer _api_key_` -> `Authorization` in headers. Please make sure to replace "*api\_key*" with actual api key from segment connector settings page.

![](/images/docs/8d6594d-image.png)

8. Now, send a test event of type `track` or `identify`. You'll see status `200 OK` response and the corresponding event on [SuprSend dashboard -> API logs](https://app.suprsend.com/en/staging/logs/api/?last_n_minutes=1440) page.
...
9. After successful testing, **Save and Enable** the mapping.

![](/images/docs/d92dc6b-image.png)

10. Also enable destination in Settings tab to start sending webhook data to SuprSend.
...
* * [Getting Started](#getting-started)
    ...
    * [1. Add Prefix to track events](#1-add-prefix-to-track-events)
    * [2. User preference mapping](#2-user-preference-mapping)
    * [3. User channel mapping](#3-user-channel-mapping)
