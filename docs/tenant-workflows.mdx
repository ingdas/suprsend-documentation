...
description: "Learn how to handle multi-tenant notifications"
---

Tenants represents a segment that user belongs to. It can be organizations, teams within an organization, subsidiary companies or different product lines in the same business. In SuprSend, you can dynamically manage tenant level notification customizations. This includes the ability to [customize template design or content](/docs/brands#using-brand-components-in-templates), set different [notification preferences at the tenant level](/docs/brand-preference), route notifications via [tenant vendors](/docs/tenant-vendor) or even have a distinct set of notifications for each tenant. All of this can be managed dynamically within SuprSend by defining tenant settings once and passing `**tenant_id**` in your event or workflow call. [Know more about how to manage tenants within SuprSend here](/docs/brands).

<Note>
  📘 If you have a usecase where the entire user base and notifications differ between two distinct business lines within your company, we recommend using different workspaces to manage these notifications effectively.&#x20;
