---
title: "Branch"
description: "Use branch to route notifications through different workflow routes based on conditions"
---
...
1. **Condition branch**
    ...
    \- User proceeds through this branch when a condition is met. The condition can be applied on recipient, actor, brand properties and node input data. You can add upto 9 condition branches. If 2 branches satisfies the condition, the first branch in the order will be executed.

2. **Default branch**

    \- This is the fallback branch which is executed if no condition branch results in truthy value.
...
| Brand / Tenant | Brand / Tenant properties. It is all the properties of the tenant\_id in your workflow trigger.                                                                                                                                      | Directly specify key if you are adding this in key or add as`$brand.<property_key>` if you add it in value field.      |

### Key-Value Pair

1. **Key**

    \- It is the variable key in your data input to apply condition on.
...
1. **Value**
   ...
   [**fixed**](/docs/branch#fixed-values)

    or 

   [**dynamic**](/docs/branch#dynamic-values)

    (evaluated based on workflow input payload). Dynamic values are helpful when the comparison value is dynamic for each user. For example, in case of payment reminder, the payment due date could vary for each user and is part of your trigger data. You can add condition in this case as 
...
* **datetime**
   ...
   \- "2024-01-01T00:00:00Z", "now" (current timestamp), "now+1d", "now-1d" (current timestamp +/- interval). You can add only 1 of these interval units in the expression (y-year, M-month, w-week, d-day, h-hour, m-minute, s-second).
...
* `OR`
   ...
   \- Evaluation will pass if any of the conditions separated by 
