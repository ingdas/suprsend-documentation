...
2. **Find translation file** → Searches using fallback hierarchy:
   - Exact match: `es-MX.json` (for `es_MX` locale)
...
1. **First try:** Exact match for the user's locale (`es-MX.json` for Spanish - Mexico)
...
```mermaid
flowchart TD
    Start["User wants translation<br/>for es-MX locale"] --> Step1["Step 1: Check es-MX.json<br/>(Spanish - Mexico)"]
...
<Note>
    **File naming:** Use correct naming convention (`en.json`, `es-MX.json`, or `auth.en.json` for namespaced files).
</Note>
  </Step>
