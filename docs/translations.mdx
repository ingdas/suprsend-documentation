...
import { Note, Accordion, Steps, Step, CodeGroup } from '@mintlify/components';
...
1. Exact locale match — e.g., `es_MX.json`
2. General language file — e.g., `es.json`
3. Default language fallback — e.g., `en.json`

```mermaid
flowchart TD
Start["User's locale is es_MX"] --> Step1["Check es_MX.json"]
...
<Steps>
  <Step title="Upload file">
    ...
    Locale file naming convention:
      - `{locale_code}.json`: example: `en.json`, `es_MX.json`
      - `{namespace}.{locale_code}.json`: example: `auth.en.json`, `orders.es_MX.json`
...
**Rendered content based on count:**
- 0 → “You have no tasks”
- 1 → “You have 1 task”
- 5 → “You have 5 tasks”
