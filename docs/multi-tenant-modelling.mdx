...
Multi-tenant platforms run a *shared notification system* across many customer organizations (tenants). The key requirement is *tenant-specific customization*: the same notification type must adapt based on tenant context—preferences, templates and localization, and routing (channel + vendor).
...
| **Orchestration Steps** | **[Workflows](/docs/workflows)** | Onboarding sequence: welcome email → first week check-in → manager introduction |
...
This approach keeps full UI and UX control with WorkVibe, fits naturally into the existing admin dashboard, and ensures each customer can configure how their organization’s notifications are sent—without leaving the WorkVibe platform.
...
**Workflows** define the orchestration logic for notifications—the sequence of steps, conditions, delays, and channels used to deliver messages. For WorkVibe we built a workflow for new employee onboarding that sends a welcome email on day one, a first-week check-in survey on day seven, and a manager introduction Slack message on day ten.
