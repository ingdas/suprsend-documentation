...
[Suggest Edits](/edit/preferences-angular)
...
2. [Channel Preferences](/docs/angular#show-channel-level-preference-section-on-the-ui) - User can set preferences at overall channel level from this section.
...
4. To update category-channel preference on the click of checkbox next to each channel, call the ***update\_channel\_preference\_in\_category*** method. Update the latest data in the instance property if no error is received in response. For preference state `opt-in` set the checkbox state as **checked** and **unchecked** for the `opt-out` state.
...
* * [Pre-requisites](#pre-requisites)
  ...
  * [Read Preferences data](#read-preferences-data)

    * [Step 1. Get Preference data](#step-1-get-preference-data)
    * [Step 2. Configure Event Listeners](#step-2-configure-event-listeners)

  * [Show Preference data on the UI](#show-preference-data-on-the-ui)

    * [Code block to Render Header and Blank state](#code-block-to-render-header-and-blank-state)
    * [Show Category level Preference section on the UI](#show-category-level-preference-section-on-the-ui)
    * [Code block to Render Category section](#code-block-to-render-category-section)
    * [Show Channel level Preference section on the UI](#show-channel-level-preference-section-on-the-ui)
    * [Code block to Render Channel section](#code-block-to-render-channel-section)

  * [Example Code to Render Full Preference Page](#example-code-to-render-full-preference-page)
