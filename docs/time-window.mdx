...
* **Trigger**
  ...
  : User updates the task status on July 22 at 9:00 PM UTC.

* **Time Window**

  : Any day, 9:00 AM - 5:00 PM.
...
1. **Day**
   ...
   : Specify the days of the week or month when the notification should be sent. Options include:
...
2. **Time**
   ...
   : Define the time within the selected days when the notification should be sent. This is calculated based on the recipient’s timezone.

3. **Timezone:**

    Set the timezone for the Time specified. You can set recipient's timezone here which will be dynamically calculated for each recipient. You can set recipient timezone in recipient profile with 
...
<AccordionGroup>
  <Accordion title="Does timezone setting take care of clock changes like day light savings?" defaultOpen={false}>
    Yes, one of the reasons for storing timezone in in [IANA (TZ identifier)](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones#List) format was to take care of these clock shifts across the globe.
  </Accordion>

  <Accordion title="What happens when I add batch node below time window?" defaultOpen={false}>
