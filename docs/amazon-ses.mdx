...
<Steps>
    ...
    **Steps to follow for setting up Event Destination and enable Event Tracking:**
    ...
    <Steps>
      <Step title="Select the Event Types">
        Go to`Event Destinations`tab and click`Add destination`In "Select event types" step, click`Select all`button to received all the events to destination.
      </Step>
      ...
      <Step title="Verify information">
        In the `Review` step, verify the info.
      </Step>

      <Step title="Click &#x22;Add destination&#x22;." />
    </Steps>
  ...
  <Step title="Give SuprSend permission to Subscribe to SNS topic." stepNumber="5">
    ...
    <Steps>
      <Step title="Step 1">
        Click on`Edit`. On the edit page, expand the`Access Policy`section. The policy looks something like below. You'll notice that the current policy gives`sns:Publish`permission to your SES account.
  ...
  <Step title="Fill Vendor settings page on Suprsend" stepNumber="6">
    ...
    4. Select 'Amazon SES' from the list Vendors.

    ![](/images/docs/b7074f4-SES.png)5. On the Vendor Settings page, enter the details in the vendor form:
